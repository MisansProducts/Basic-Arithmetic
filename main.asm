; Made by Alex

section	.data
; Constants
NULL		equ	0

TRUE		equ	1
FALSE		equ	0

EXIT_SUCCESS	equ	0
SYS_exit	equ	60

; Byte data
bNum1		db	11
bNum2		db	15
bNum3		db	26
bNum4		db	37
bNum5		db	33
bNum6		db	-17
bNum7		db	-29
bNum8		db	-40
bAns1		db	0
bAns2		db	0
bAns3		db	0
bAns4		db	0
bAns5		db	0
bAns6		db	0
bAns7		db	0
bAns8		db	0
bAns9		db	0
bAns10		db	0
wAns11		dw	0
wAns12		dw	0
wAns13		dw	0
wAns14		dw	0
wAns15		dw	0
bAns16		db	0
bAns17		db	0
bAns18		db	0
bRem18		db	0
bAns19		db	0
bAns20		db	0
bAns21		db	0
bRem21		db	0

; Word data
wNum1		dw	229
wNum2		dw	467
wNum3		dw	1738
wNum4		dw	2210
wNum5		dw	375
wNum6		dw	-234
wNum7		dw	-361
wNum8		dw	-418
wAns1		dw	0
wAns2		dw	0
wAns3		dw	0
wAns4		dw	0
wAns5		dw	0
wAns6		dw	0
wAns7		dw	0
wAns8		dw	0
wAns9		dw	0
wAns10		dw	0
dAns11		dd	0
dAns12		dd	0
dAns13		dd	0
dAns14		dd	0
dAns15		dd	0
wAns16		dw	0
wAns17		dw	0
wAns18		dw	0
wRem18		dw	0
wAns19		dw	0
wAns20		dw	0
wAns21		dw	0
wRem21		dw	0

; Double word data
dNum1		dd	13456789
dNum2		dd	31342
dNum3		dd	219815
dNum4		dd	61569
dNum5		dd	2617
dNum6		dd	-1345
dNum7		dd	-2733
dNum8		dd	-4256
dAns1		dd	0
dAns2		dd	0
dAns3		dd	0
dAns4		dd	0
dAns5		dd	0
dAns6		dd	0
dAns7		dd	0
dAns8		dd	0
dAns9		dd	0
dAns10		dd	0
qAns11		dq	0
qAns12		dq	0
qAns13		dq	0
qAns14		dq	0
qAns15		dq	0
dAns16		dd	0
dAns17		dd	0
dAns18		dd	0
dRem18		dd	0
dAns19		dd	0
dAns20		dd	0
dAns21		dd	0
dRem21		dd	0

; Quad word data
qNum1		dq	24656793
qNum2		dq	115732
qNum3		dq	1526241
qNum4		dq	254879
qNum5		dq	317517
qNum6		dq	-222147
qNum7		dq	-216517
qNum8		dq	-445758
qAns1		dq	0
qAns2		dq	0
qAns3		dq	0
qAns4		dq	0
qAns5		dq	0
qAns6		dq	0
qAns7		dq	0
qAns8		dq	0
qAns9		dq	0
qAns10		dq	0
dqAns11		ddq	0
dqAns12		ddq	0
dqAns13		ddq	0
dqAns14		ddq	0
dqAns15		ddq	0
qAns16		dq	0
qAns17		dq	0
qAns18		dq	0
qRem18		dq	0
qAns19		dq	0
qAns20		dq	0
qAns21		dq	0
qRem21		dq	0

section	.text
global _start
_start:

; =-=-= Byte arithmetic =-=-=

; ADDITION
; UNSIGNED (+/-)
; bAns1 = bNum1 + bNum4
mov al, byte[bNum1] ; Move to lower 8 bit register
add al, byte[bNum4] ; Perform addition
mov byte[bAns1], al ; Save answer
; bAns2 = bNum2 + bNum4
mov al, byte[bNum2]
add al, byte[bNum4]
mov byte[bAns2], al
; bAns3 = bNum3 + bNum3
mov al, byte[bNum3]
add al, byte[bNum3]
mov byte[bAns3], al
; SIGNED (+)
; bAns4 = bNum5 + bNum6
mov al, byte[bNum5]
add al, byte[bNum6]
mov byte[bAns4], al
; bAns5 = bNum7 + bNum8
mov al, byte[bNum7]
add al, byte[bNum8]
mov byte[bAns5], al

; SUBTRACTION
; UNSIGNED (+/-)
; bAns6 = bNum4 - bNum3
mov al, byte[bNum4]
sub al, byte[bNum3]
mov byte[bAns6], al
; bAns7 = bNum3 - bNum1
mov al, byte[bNum3]
sub al, byte[bNum1]
mov byte[bAns7], al
; bAns8 = bNum2 - bNum1
mov al, byte[bNum2]
sub al, byte[bNum1]
mov byte[bAns8], al
; SIGNED (+)
; bAns9 = bNum6 - bNum6
mov al, byte[bNum6]
sub al, byte[bNum6]
mov byte[bAns9], al
; bAns10 = bNum8 - bNum7
mov al, byte[bNum8]
sub al, byte[bNum7]
mov byte[bAns10], al

; MULTIPLICATION
; UNSIGNED (+/-)
; wAns11 = bNum1 * bNum2
mov al, byte[bNum1] ; Move to lower 8 bit register
mul byte[bNum2] ; Answer is stored in ah:al or ax
mov word[wAns11], ax ; Save answer
; wAns12 = bNum3 * bNum4
mov al, byte[bNum3]
mul byte[bNum4]
mov word[wAns12], ax
; wAns13 = bNum3 * bNum3
mov al, byte[bNum3]
mul byte[bNum3]
mov word[wAns13], ax
; SIGNED (+)
; wAns14 = bNum5 * bNum8
mov al, byte[bNum5]
imul byte[bNum8]
mov word[wAns14], ax
; wAns15 = bNum6 * bNum7
mov al, byte[bNum6]
imul byte[bNum7]
mov word[wAns15], ax

; DIVISION
; UNSIGNED (+/-)
; bAns16 = bNum4 / bNum2
mov ah, 0 ; ah is sign extended since bNum4 can fit in al
mov al, byte[bNum4] ; Dividend must be ah:al or ax
div byte[bNum2] ; Answer is stored in al
mov byte[bAns16], al ; Save answer
; bAns17 = bNum3 / bNum1
mov ah, 0
mov al, byte[bNum3]
div byte[bNum1]
mov byte[bAns17], al
; bAns18 = wNum4 / bNum1
mov ax, word[wNum4]
div byte[bNum1]
mov byte[bAns18], al
; bRem18 = wNum4 % bNum1
mov byte[bRem18], ah
; SIGNED (+)
; bAns19 = bNum5 / bNum7
mov al, byte[bNum5]
cbw ; Converts byte to word
idiv byte[bNum7]
mov byte[bAns19], al
; bAns20 = bNum8 / bNum6
mov al, byte[bNum8]
cbw
idiv byte[bNum6]
mov byte[bAns20], al
; bAns21 = wNum7 / bNum6
mov ax, word[wNum7]
idiv byte[bNum6]
mov byte[bAns21], al
; bRem21 = wNum7 % bNum6
mov byte[bRem21], ah

; =-=-= Word arithmetic =-=-=

; ADDITION
; UNSIGNED (+/-)
; wAns1 = wNum2 + wNum3
mov ax, word[wNum2]
add ax, word[wNum3]
mov word[wAns1], ax
; wAns2 = wNum1 + wNum4
mov ax, word[wNum1]
add ax, word[wNum4]
mov word[wAns2], ax
; wAns3 = wNum2 + wNum1
mov ax, word[wNum2]
add ax, word[wNum1]
mov word[wAns3], ax
; SIGNED (+)
; wAns4 = wNum5 + wNum8
mov ax, word[wNum5]
add ax, word[wNum8]
mov word[wAns4], ax
; wAns5 = wNum6 + wNum7
mov ax, word[wNum6]
add ax, word[wNum7]
mov word[wAns5], ax

; SUBTRACTION
; UNSIGNED (+/-)
; wAns6 = wNum4 - wNum1
mov ax, word[wNum4]
sub ax, word[wNum1]
mov word[wAns6], ax
; wAns7 = wNum3 - wNum2
mov ax, word[wNum3]
sub ax, word[wNum2]
mov word[wAns7], ax
; wAns8 = wNum2 - wNum1
mov ax, word[wNum2]
sub ax, word[wNum1]
mov word[wAns8], ax
; SIGNED (+)
; wAns9 = wNum5 - wNum7
mov ax, word[wNum5]
sub ax, word[wNum7]
mov word[wAns9], ax
; wAns10 = wNum8 - wNum6
mov ax, word[wNum8]
sub ax, word[wNum6]
mov word[wAns10], ax

; MULTIPLICATION
; UNSIGNED (+/-)
; dAns11 = wNum1 * wNum3
mov ax, word[wNum1]
mul word[wNum3]
mov word[dAns11], ax
mov word[dAns11 + 2], dx
; dAns12 = wNum4 * wNum1
mov ax, word[wNum4]
mul word[wNum1]
mov word[dAns12], ax
mov word[dAns12 + 2], dx
; dAns13 = wNum2 * wNum2
mov ax, word[wNum2]
mul word[wNum2]
mov word[dAns13], ax
mov word[dAns13 + 2], dx
; SIGNED (+)
; dAns14 = wNum6 * wNum7
mov ax, word[wNum6]
imul word[wNum7]
mov word[dAns14], ax
mov word[dAns14 + 2], dx
; dAns15 = wNum5 * wNum8
mov ax, word[wNum5]
imul word[wNum8]
mov word[dAns15], ax
mov word[dAns15 + 2], dx

; DIVISION
; UNSIGNED (+/-)
; wAns16 = wNum4 / wNum2
mov dx, 0 ; Sign extend dx
mov ax, word[wNum4] ; Dividend must be dx:ax
div word[wNum2] ; Answer is stored in ax
mov word[wAns16], ax ; Save answer
; wAns17 = wNum3 / wNum1
mov dx, 0
mov ax, word[wNum3]
div word[wNum1]
mov word[wAns17], ax
; wAns18 = dNum4 / wNum3
mov ax, word[dNum4]
mov dx, word[dNum4 + 2] ; Need to use the same size type, so split the variable
div word[wNum3]
mov word[wAns18], ax
; wRem18 = dNum4 % wNum3
mov word[wRem18], dx
; SIGNED (+)
; wAns19 = wNum5 / wNum7
mov ax, word[wNum5]
cwd ; Converts word to dword
idiv word[wNum7]
mov word[wAns19], ax
; wAns20 = wNum7 / wNum6
mov ax, word[wNum7]
cwd
idiv word[wNum6]
mov word[wAns20], ax
; wAns21 = dNum8 / wNum6
mov ax, word[dNum8]
mov dx, word[dNum8 + 2]
idiv word[wNum6]
mov word[wAns21], ax
; wRem21 = dNum8 % wNum6
mov word[wRem21], dx

; =-=-= Double word arithmetic =-=-=

; ADDITION
; UNSIGNED (+/-)
; dAns1 = dNum1 + dNum3
mov eax, dword[dNum1]
add eax, dword[dNum3]
mov dword[dAns1], eax
; dAns2 = dNum4 + dNum3
mov eax, dword[dNum4]
add eax, dword[dNum3]
mov dword[dAns2], eax
; dAns3 = dNum2 + dNum4
mov eax, dword[dNum2]
add eax, dword[dNum4]
mov dword[dAns3], eax
; SIGNED (+)
; dAns4 = dNum7 + dNum5 
mov eax, dword[dNum7]
add eax, dword[dNum5]
mov dword[dAns4], eax
; dAns5 = dNum8 + dNum6
mov eax, dword[dNum8]
add eax, dword[dNum6]
mov dword[dAns5], eax

; SUBTRACTION
; UNSIGNED (+/-)
; dAns6 = dNum1 - dNum2
mov eax, dword[dNum1]
sub eax, dword[dNum2]
mov dword[dAns6], eax
; dAns7 = dNum4 - dNum2
mov eax, dword[dNum4]
sub eax, dword[dNum2]
mov dword[dAns7], eax
; dAns8 = dNum3 - dNum1
mov eax, dword[dNum3]
sub eax, dword[dNum1]
mov dword[dAns8], eax
; SIGNED (+)
; dAns9 = dNum5 - dNum8
mov eax, dword[dNum5]
sub eax, dword[dNum8]
mov dword[dAns9], eax
; dAns10 = dNum7 - dNum6
mov eax, dword[dNum7]
sub eax, dword[dNum6]
mov dword[dAns10], eax

; MULTIPLICATION
; UNSIGNED (+/-)
; qAns11 = dNum4 * dNum1
mov eax, dword[dNum4]
mul dword[dNum1]
mov dword[qAns11], eax
mov dword[qAns11 + 4], edx
; qAns12 = dNum3 * dNum3
mov eax, dword[dNum3]
mul dword[dNum3]
mov dword[qAns12], eax
mov dword[qAns12 + 4], edx
; qAns13 = dNum2 * dNum1
mov eax, dword[dNum2]
mul dword[dNum1]
mov dword[qAns13], eax
mov dword[qAns13 + 4], edx
; SIGNED (+)
; qAns14 = dNum7 * dNum5
mov eax, dword[dNum7]
imul dword[dNum5]
mov dword[qAns14], eax
mov dword[qAns14 + 4], edx
; qAns15 = dNum8 * dNum6
mov eax, dword[dNum8]
imul dword[dNum6]
mov dword[qAns15], eax
mov dword[qAns15 + 4], edx

; DIVISION
; UNSIGNED (+/-)
; dAns16 = dNum1 / dNum3
mov edx, 0
mov eax, dword[dNum1]
div dword[dNum3]
mov dword[dAns16], eax
; dAns17 = dNum4 / dNum2
mov edx, 0
mov eax, dword[dNum4]
div dword[dNum2]
mov dword[dAns17], eax
; dAns18 = qAns12 / dNum3
mov eax, dword[qAns12]
mov edx, dword[qAns12 + 4]
div dword[dNum3]
mov dword[dAns18], eax
; dRem18 = qAns12 % dNum3
mov dword[dRem18], edx
; SIGNED (+)
; dAns19 = dNum5 / dNum7
mov eax, dword[dNum5]
cdq ; Converts dword to qword
idiv dword[dNum7]
mov dword[dAns19], eax
; dAns20 = dNum8 / dNum6
mov eax, dword[dNum8]
cdq
idiv dword[dNum6]
mov dword[dAns20], eax
; dAns21 = qAns11 / dNum7
mov eax, dword[qAns11]
mov edx, dword[qAns11 + 4]
idiv dword[dNum7]
mov dword[dAns21], eax
; dRem21 = qAns11 % dNum7
mov dword[dRem21], edx

; =-=-= Quad word arithmetic =-=-=

; ADDITION
; UNSIGNED (+/-)
; qAns1 = qNum1 + qNum2
mov rax, qword[qNum1]
add rax, qword[qNum2]
mov qword[qAns1], rax
; qAns2 = qNum3 + qNum4
mov rax, qword[qNum3]
add rax, qword[qNum4]
mov qword[qAns2], rax
; qAns3 = qNum3 + qNum4
mov rax, qword[qNum3]
add rax, qword[qNum4]
mov qword[qAns3], rax
; SIGNED (+)
; qAns4 = qNum5 + qNum7
mov rax, qword[qNum5]
add rax, qword[qNum7]
mov qword[qAns4], rax
; qAns5 = qNum6 + qNum8
mov rax, qword[qNum6]
add rax, qword[qNum8]
mov qword[qAns5], rax

; SUBTRACTION
; UNSIGNED (+/-)
; qAns6 = qNum4 - qNum1
mov rax, qword[qNum4]
sub rax, qword[qNum1]
mov qword[qAns6], rax
; qAns7 = qNum3 - qNum2
mov rax, qword[qNum3]
sub rax, qword[qNum2]
mov qword[qAns7], rax
; qAns8 = qNum2 - qNum1
mov rax, qword[qNum2]
sub rax, qword[qNum1]
mov qword[qAns8], rax
; SIGNED (+)
; qAns9 = qNum8 - qNum6
mov rax, qword[qNum8]
sub rax, qword[qNum6]
mov qword[qAns9], rax
; qAns10 = qNum5 - qNum7
mov rax, qword[qNum5]
sub rax, qword[qNum7]
mov qword[qAns10], rax

; MULTIPLICATION
; UNSIGNED (+/-)
; dqAns11 = qNum1 * qNum3
mov rax, qword[qNum1]
mul qword[qNum3]
mov qword[dqAns11], rax
mov qword[dqAns11 + 8], rdx
; dqAns12 = qNum2 * qNum4
mov rax, qword[qNum2]
mul qword[qNum4]
mov qword[dqAns12], rax
mov qword[dqAns12 + 8], rdx
; dqAns13 = qNum1 * qNum2
mov rax, qword[qNum1]
mul qword[qNum2]
mov qword[dqAns13], rax
mov qword[dqAns13 + 8], rdx
; SIGNED (+)
; dqAns14 = qNum5 * qNum7
mov rax, qword[qNum5]
imul qword[qNum7]
mov qword[dqAns14], rax
mov qword[dqAns14 + 8], rdx
; dqAns15 = qNum6 * qNum8
mov rax, qword[qNum6]
imul qword[qNum8]
mov qword[dqAns15], rax
mov qword[dqAns15 + 8], rdx

; DIVISION
; UNSIGNED (+/-)
; qAns16 = qNum4 / qNum1
mov rdx, 0
mov rax, qword[qNum4]
div qword[qNum1]
mov qword[qAns16], rax
; qAns17 = qNum3 / qNum2
mov rdx, 0
mov rax, qword[qNum3]
div qword[qNum2]
mov qword[qAns17], rax
; qAns18 = dqAns12 / qNum3
mov rax, qword[dqAns12]
mov rdx, qword[dqAns12 + 8]
div qword[qNum3]
mov qword[qAns18], rax
; qRem18 = dqAns12 % qNum3
mov qword[qRem18], rdx
; SIGNED (+)
; qAns19 = qNum5 / qNum7
mov rax, qword[qNum5]
cqo ; Converts qword to oword
idiv qword[qNum7]
mov qword[qAns19], rax
; qAns20 = qNum8 / qNum6
mov rax, qword[qNum8]
cqo
idiv qword[qNum6]
mov qword[qAns20], rax
; qAns21 = dqAns11 / qNum7
mov rax, qword[dqAns11]
mov rdx, qword[dqAns11 + 8]
idiv qword[qNum7]
mov qword[qAns21], rax
; qRem21 = dqAns11 % qNum7
mov qword[qRem21], rdx

last:
	mov	rax, SYS_exit
	mov	rdi, EXIT_SUCCESS
	syscall
